...
while err > 1e-3
    ...
    Ubar(:,i) = Un(:,i).....
        -(dt/dx)*(F(:,i)-F(:,i-1)).......
        +dt*S(:,i)......
        +C*abs((p(:,i+1)-2*p(:,i)+p(:,i-1))/(p(:,i+1).....
        +2*p(:,i)+p(:,i-1)))*(U(:,i+1)-2*U(:,i)+U(:,i-1));
    ...
    Ubarbar(:,i) = Un(:,i).....
        ...
        +dt*Sbar(:,i)....
        +C*abs((p(:,i+1)-2*p(:,i)+p(:,i-1))/(p(:,i+1)....
        +2*p(:,i)+p(:,i-1)))*(Ubar(:,i+1)-2*Ubar(:,i)+Ubar(:,i-1));
    ...
    %plot(x,V./sqrt(k*R*T),'-o',x,findExact(x,A)); pause(0.00001);
end
plot(x,V./sqrt(k*R*T),'-o',x,findExact(x,A));pause(0.1);
% FE=findExact(x,A)';%Find Exact
% MC=V./sqrt(k*R*T)%My Code
% L=length(FE);
% FEE=FE(L-15:L);
% MCC=MC(L-15:L);
% E(:,Q)=sum(abs(FEE-MC(L-15:L)));
% end
