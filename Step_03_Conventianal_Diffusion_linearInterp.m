clear; clc;
nu = 0.02; delta = 6*nu;
N1 = 32; x1 = linspace(0,1-delta,N1+1);
N2 = 40; x2 = linspace(1-delta,1,N2+1);
N = N1+N2;
x = [x1,x2(2:end)];
Delta_x = diff(x);
...
i = 2:N+1;
a(i) = 2*nu./dx(i-1)+Delta_x(i)/dx(i-1);
b(i) = -(2*nu*(1./dx(i-1)+1./dx(i))....
    + Delta_x(i+1)/dx(i)-Delta_x(i-1)/dx(i-1));
c(i) = 2*nu./dx(i)-Delta_x(i)/dx(i);
